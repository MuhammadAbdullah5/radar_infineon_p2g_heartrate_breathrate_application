...
void com_close(int32_t port_handle)
{
    ...
    if ((port_handle >= 0) && // port handle ranges in 0 and maximum number of allocated handles
        (port_handle < (int32_t)num_allocated_handles) &&
        (handles[port_handle] != INVALID_HANDLE_VALUE)) // if it's not already invalid that means we are trying to close an already closed com port
    {
